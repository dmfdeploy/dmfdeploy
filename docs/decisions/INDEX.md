...
| [0031](0031-oss-v0-1-sandbox-and-aws-release-profile-matrix.md) | OSS v0.1 release-profile matrix — canonical workflow contract with two lanes: `sandbox-single-node` (release gate) + `aws-arm64-multi-node` (parallel, eligible-if-ready); live Hetzner lab (`<env>`) explicitly preserved as reference, not release. Original AWS-only draft preserved at §Historical | Accepted | release / provider / security |
| [0032](0032-catalog-launcher-scoped-netbox-writer.md) | Catalog launchers mutate NetBox via scoped writer service accounts (`dmf-catalog-svc` for create/patch, `dmf-catalog-purge-svc` for delete-only), never the admin token; lifecycle tags pre-created at bootstrap. Refines ADR-0028 C3 (least-privilege machine identity) | Accepted (amended 2026-08-13 — `delete` added on `ipam.service`+`extras.tag` for `media-finalise-purge`; amended again 2026-08-14 — that `delete` object-scoped to workload-tagged records; amended a third time, same day — `delete` moved onto a new delete-only identity, `dmf-catalog-purge-svc`; amended a fourth time, 2026-08-16 — `view` on `dcim.site` added for the L3 topology guard's site-agreement read) | security / operations |
...
| [0042](0042-canonical-cross-agent-skills.md) | One canonical agent-neutral skills store at `.agents/skills/` (the cross-agent native path — Codex/Gemini/Cursor read it directly); per-agent `.claude/`/`.qwen/` views **generated** by `bin/sync-skills.sh` + gitignored, CI `skills-sync` gate on drift. Auto-minted skills land in a gitignored `_inbox/` and are PR-promoted (capture→review→promote); sync-by-default with `agents:`/`visibility:` frontmatter tiers. Delivers the decision half of #46; resolves part of #26 | Accepted | architecture / agents / tooling |
| [0043](0043-workload-scale-to-zero-availability.md) | Workload scale-to-zero is an **availability** action (replicas 0↔N), **scoped to AWX only** — the `awx-presence` role (bootstrap 1→0→1→0) + `awx-autoscale` helper (steady-state wake/idle-reap). The seam is named for legibility but **not generalized**: any other workload needs a new ADR. Hard boundary — AWX stays the actuator (ADR-0013/0025/0037); the helper changes availability only, never job/catalog/media semantics (RBAC-enforced). Distinct from node elasticity + cost guardrails (deferred). Refs #97 | Accepted | architecture / availability / awx |
| [0044](0044-dmf-init-stateless-tmpfs-recovery-model.md) | dmf-init stays **stateless**: no env state on host disk, `DMF_DATA_ROOT` **must** be tmpfs (enforced fail-closed at startup, not warn-only — closes the silent secrets-to-disk gap). The disk-backed resume cursor is tmpfs-only, **not** durable across `docker rm`; cross-lifetime/crash recovery is **rollback to the last *exported* checkpoint** (downloaded bundle), so the design must force checkpoint export before long unattended phases. Host-mounted durable cursor rejected. Tightens ADR-0036; implements facet (c) of #150 (depends on facet (b) first) | Accepted (2026-07-02 — landed in dmf-init v0.3.5, #150/#162 closed) | architecture / dmf-init / recovery |
| [0045](0045-media-function-licensing-reservable-resource.md) | Media-function **licences are a reservable resource behind a `LicenceReservationProvider` seam** (`check`/`reserve`/`release`/`usage`; catalog declares required licence classes; `reservation_id` threaded to launchers; idempotent release on finalise/failed-launch; deny-on-exhaustion), **not** ad-hoc NetBox tags (a tag can't hold counts/leases/rollback). v0.2 = **declared-but-not-enforced** (reserve the `licence:` catalog fields, build no provider, ship no mock pool); a real provider is a later deliberately-chosen slice. NetBox is not the reservation ledger. Feeds the v0.2 EBU re-anchor plan | Proposed (RFC pending) | architecture / media-functions / licensing |
| [0046](0046-first-class-media-workload-entity.md) | A **Media Workload** is a first-class **tag-derived grouping** of Media Function instances (NetBox `workload:<slug>`, exactly **0-or-1 per service**; zero → explicit `unassigned`) — no NetBox graph object. The six-stage EBU lifecycle is **workload-level** (functions carry run intent); v0.2 derives only Provision/Configure/Operate and joins health by **service/instance identity**, not the `app` rollup. Launchers **must preserve non-owned tags** so `workload:*` survives deploy (hard prerequisite). Reserve "Media Workloads" for the assembly, rename the flat surface to "Media Functions", split the API additively. Narrow vertical fix: only media-processing entries omit `vertical` + gain a type/role. Amends ADR-0037; converged in Discussion #190 | Accepted | architecture / media-functions / netbox |
| [0047](0047-media-function-packaging-catalog-sources-trust-model.md) | Third-party media functions enter only as **signed, digest-pinned Media Function Packages** (Helm mandatory; apps, not cluster machinery) from classed catalog sources — `project` / `vendor` / `community` — under **four separate axes** (source class, promotion state `Draft→…→Revoked`, support attribute, launcher privilege). **Bespoke launchers are `project`-source-only**; everything else gets the generic declarative launcher under versioned `generic-chart-policy/v1` (normative appendix, conformance fixtures). Two-maintainer promotion quorum, one-maintainer emergency revoke; revocation = signed fail-closed deny-list consumed at every entry path; air-gap updates via signed offline bundle (revocations first). Vendor deployability gated on the generic launcher actually existing; community = browse-only + audited trust-bypass on quarantined nodes. cosign/Sigstore at promotion; licence-presence enforcement deferred to ADR-0045. Converged in Discussion #248 | Accepted | architecture / media-functions / catalog / trust |
## Commitments
